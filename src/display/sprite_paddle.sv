module sprite_paddle #(
    parameter FILE = "data/paddle_left.hex",
    parameter PALETTE_SIZE_BITS = 4,
    parameter PADDLE_X = 100,
    ...
    localparam ROM_X_BITS = 4;
    localparam ROM_Y_BITS = 7;

    reg [SCREEN_H_BITS-1:0] pos_y;

    always @(posedge I_clk_pixel or negedge I_rst_n) begin
        if (!I_rst_n) pos_y <= SCREEN_H_BITS'(0);
        else pos_y <= I_new_pos_y;
    end
    ...
    sprite_sync_coord #(
        ...
        .I_sprite_x(SCREEN_W_BITS'(PADDLE_X)),
        .I_sprite_y(pos_y),
